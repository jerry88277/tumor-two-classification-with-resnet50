...
# SEED = 4019
    ...
    images_dir = os.path.join(ROOT, 'images2')
    train_dir = os.path.join(ROOT, 'train2')
    val_dir = os.path.join(ROOT, 'val2')
    test_dir = os.path.join(ROOT, 'test2')
    ...
    for epoch in range(EPOCHS):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), f'2Ctut5-model{SEED}.pt')
    ...
    plt.xlabel("epochs")
    plt.legend()
    plt.savefig('2ClearningCurve.png')
    plt.close()
    ...
    plt.xlabel("epochs") 
    plt.legend()
    plt.savefig('2CAccuracy_Top1.png')
    plt.close()
    ...
    plt.xlabel("epochs") 
    plt.legend()
    plt.savefig('2CAccuracy_Top3.png')
    plt.close()
    
    # In[] test
    
    model.load_state_dict(torch.load(f'2Ctut5-model{SEED}.pt'))
    ...
    for iterator, datatype in iterator_list:
        ...
        predict_result.to_csv(f'prediction/{datatype}_2Cpredict_result_seed{SEED}.csv')
        
    metrice_result.reset_index(drop=True)
    metrice_result.to_csv(f'result/2Cmetrice_result_seed{SEED}.csv')
